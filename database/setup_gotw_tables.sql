...
CREATE POLICY "Allow users to update their own votes" ON gotw_votes
    FOR UPDATE USING (auth.role() = 'authenticated' AND user_id::text = auth.uid()::text);

CREATE POLICY "Allow users to delete their own votes" ON gotw_votes
    FOR DELETE USING (auth.role() = 'authenticated' AND user_id::text = auth.uid()::text);
